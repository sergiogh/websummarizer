...
class SpreadsheetConnector:
    ...
    def get_content(self, delta=7, start_date=None, end_date=None) -> None:
        try:
            ...
            response.raise_for_status()  # This will raise an exception for any HTTP error
            ...
            date_column_index = 3  # Date is now in the 4th column (0-based index)
            ...
            for row in filtered_rows:
                if len(row) >= 4:  # Make sure row has enough columns
                    self.titles.append(row[0])  # Title is in the 1st column
                    self.urls.append(row[1])    # URL is in the 2nd column
                    self.tags.append(row[2])    # Tag is in the 3rd column
                    self.published_at.append(row[3])  # Date is in the 4th column
                else:
                    logger.warning(f"Skipping row due to insufficient columns: {row}")
